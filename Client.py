...
# ─── BRAWLER DISPLAY DATA (visuals/UI only — no damage/speed here) ─────────────
# The server owns all combat stats. This dict is purely for rendering.
BRAWLERS = {
    "sniper": {
        "weapon": "sniper",
...
# Maps server weapon string → brawler name for rendering other players
WEAPON_TO_BRAWLER = {
    "sniper":   "sniper",
    "minigun":  "minigun",
    "magic":    "mage",
...
def draw_brawler(surf, brawler_name, x, y, alpha=255, scale=1.0, starpower=False, alive=True):
    ...
    if brawler_name == "sniper":
        ...
        # Thick rounded square — beefy, armored look
        hw = int(r * 0.85)
        pygame.draw.rect(tmp, ac(color),  (cx-hw, cy-hw, hw*2, hw*2), border_radius=5)
        pygame.draw.rect(tmp, ac(accent), (cx-hw, cy-hw, hw*2, hw*2), 3, border_radius=5)
        # Bolts in corners
        for bx2, by2 in [(cx-hw+4, cy-hw+4), (cx+hw-4, cy-hw+4),
                         (cx-hw+4, cy+hw-4), (cx+hw-4, cy+hw-4)]:
            pygame.draw.circle(tmp, ac(accent), (bx2, by2), 3)
        # Barrel
        pygame.draw.rect(tmp, ac(accent), (cx, cy - 3, r + 4, 6), border_radius=2)
        if starpower:
            # Glow ring
            gs = pygame.Surface((r*2+16, r*2+16), pygame.SRCALPHA)
            now = pygame.time.get_ticks()
            ga = int(60 + 30*math.sin(now*0.008))
            pygame.draw.rect(gs, (*accent, ga), (0, 0, r*2+16, r*2+16), border_radius=8)
            tmp.blit(gs, (cx - r - 8, cy - r - 8))

    elif brawler_name == "ninja":
        # Slim pointed octagon — agile, sharp
        now = pygame.time.get_ticks()
        spin = now * 0.003 if starpower else 0
        pts = []
        for i in range(8):
            ang = math.radians(45*i - 90) + spin
            rad = r if i % 2 == 0 else int(r * 0.6)
            pts.append((int(cx + rad*math.cos(ang)), int(cy + rad*math.sin(ang))))
        pygame.draw.polygon(tmp, ac(color), pts)
        pygame.draw.polygon(tmp, ac(accent), pts, 2)
        # Crossed blades in center
        pygame.draw.line(tmp, ac(accent), (cx-r//2, cy-r//2), (cx+r//2, cy+r//2), 2)
        pygame.draw.line(tmp, ac(accent), (cx+r//2, cy-r//2), (cx-r//2, cy+r//2), 2)
        if starpower:
            # Speed trail dots
            for i in range(3):
                ta = int(80 - i*25)
                pygame.draw.circle(tmp, (*accent, ta), (cx - i*4, cy + i*3), max(1, r//3 - i))

    elif brawler_name == "healer":
        # Cross / plus shape — medical symbol
        arm = int(r * 0.4)
        # Vertical bar
        pygame.draw.rect(tmp, ac(color), (cx - arm, cy - r, arm*2, r*2), border_radius=4)
        # Horizontal bar
        pygame.draw.rect(tmp, ac(color), (cx - r, cy - arm, r*2, arm*2), border_radius=4)
        # Outline
        pygame.draw.rect(tmp, ac(accent), (cx - arm, cy - r, arm*2, r*2), 2, border_radius=4)
        pygame.draw.rect(tmp, ac(accent), (cx - r, cy - arm, r*2, arm*2), 2, border_radius=4)
        # Pulsing core
        now = pygame.time.get_ticks()
        pulse = int(4 + 3*math.sin(now * 0.005))
        pygame.draw.circle(tmp, ac(accent), (cx, cy), pulse)
        if starpower:
            # Regen particles
            for i in range(4):
                ang = math.radians(90*i + now*0.004)
                dx2 = int(cx + r*0.7*math.cos(ang))
                dy2 = int(cy + r*0.7*math.sin(ang))
                pygame.draw.circle(tmp, ac(accent, 160), (dx2, dy2), 3)
    else:
...
BULLET_STYLE = {
    "sniper":   {"color": (100, 220, 255), "size": 6,  "glow": (80, 200, 255)},
    "minigun":  {"color": (255, 200,  50), "size": 4,  "glow": (255, 160,  30)},
    "magic":    {"color": (220, 100, 255), "size": 6,  "glow": (180,  60, 255)},
    "shotgun":  {"color": (255, 120,  40), "size": 5,  "glow": (255,  80,  20)},
    "shuriken": {"color": (100, 255, 200), "size": 5,  "glow": ( 60, 220, 160)},
    "orb":      {"color": (120, 255, 100), "size": 6,  "glow": ( 80, 200,  60)},
...
def send_brawler_select(bname):
    # Only send brawler name — server looks up weapon from its own dict
...
def draw_name_entry():
    ...
    # Show all 6 brawlers in preview row
...
def draw_lobby():
    ...
    # ── CENTER: Brawler selection (all 6) ──
    ...
    brawler_names = list(BRAWLERS.keys())
    # Fit all 6 cards — two rows of 3 if needed, else one row
    ...
    for idx, bname in enumerate(brawler_names):
        ...
        art = pygame.Surface((90, 90), pygame.SRCALPHA)
        draw_brawler(art, bname, 45, 45, scale=2.2, starpower=sel)
        screen.blit(art, (bx + card_w//2 - 45, card_y + 12 + bob))
        ...
        screen.blit(ns, (bx + card_w//2 - ns.get_width()//2, card_y + 113))
    ...
    art2 = pygame.Surface((90, 90), pygame.SRCALPHA)
    draw_brawler(art2, selected_brawler, 45, 45, scale=2.4, starpower=ready_local)
    screen.blit(art2, (right_x + right_w//2 - 45, panel_y + 14))
    ...
    pygame.draw.line(screen, (24, 28, 50), (right_x + 12, panel_y + 210),
                     (right_x + right_w - 12, panel_y + 210), 1)
...
def try_shoot():
    ...
    # Client only sends aim direction — NO damage, speed, or weapon stats
    ...
    now = pygame.time.get_ticks()
    # Local cooldown is just to avoid flooding — server enforces the real rate
    if now - last_shot < 50:
        return
    last_shot = now
    play_sound("shoot")
    send({
        ...
        "dx":   math.cos(ang),
        "dy":   math.sin(ang),
        # NO damage, NO speed, NO weapon — server uses its authoritative values
    })
...
def draw_game():
    ...
    # ── Bullets — color based on weapon from server data ──
    for b in server_bullets:
        ...
        gcol = style["glow"]
        pygame.draw.circle(gs2, (*gcol, 55), (11, 11), 10)
